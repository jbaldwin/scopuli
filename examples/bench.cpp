...
int main(int argc, char *argv[]) {
  ...
  constexpr option long_options[] = {{"version", no_argument, nullptr, 'v'},
  ...
  while ((opt = getopt_long(argc, argv, short_options, long_options,
    ...
    case 'h':
      ...
      return EXIT_SUCCESS;
    default:
      spdlog::error("Unknown option provided: [{}], exiting.",
  ...
  scopuli::http::server server{
      coro::net::ip_address::from_string("127.0.0.1"), 8080,
      [](const scopuli::http::request &req,
         scopuli::http::response &rep) -> void {
