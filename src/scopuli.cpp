...
{
server::server(coro::net::ip_address ip_addr, uint16_t port, routes_type routes, processing_method pm)
    : m_ip_addr(std::move(ip_addr)),
      m_port(port),
      m_routes(std::move(routes))
{
    size_t scheduler_count    = 1;
    auto   execution_strategy = coro::io_scheduler::execution_strategy_t::process_tasks_on_thread_pool;
    ...
    {
        m_io_schedulers.push_back(std::make_unique<coro::io_scheduler>(coro::io_scheduler::options{
            .thread_strategy            = coro::io_scheduler::thread_strategy_t::spawn,
            .on_io_thread_start_functor = nullptr,
            .on_io_thread_stop_functor  = nullptr,
            .pool = coro::thread_pool::options{.on_thread_start_functor = nullptr, .on_thread_stop_functor = nullptr},
            .execution_strategy = execution_strategy}));

        m_accept_tasks.emplace_back(make_accept_task(*m_io_schedulers.back().get(), ip_addr, port));
    }
...
auto server::make_accept_task(coro::io_scheduler& scheduler, coro::net::ip_address ip_addr, uint16_t port)
    -> coro::task<void>
{
    ...
    coro::net::tcp_server tcp_server{scheduler, coro::net::tcp_server::options{.address = ip_addr, .port = port}};
    ...
    while (!m_shutdown.load(std::memory_order::acquire))
    {
        ...
        if (pstatus == coro::poll_status::event)
        {
            ...
            {
                spdlog::error("scopuli::http::server error accepting incoming connection");
            }
        }
        else
        {
    ...
    {
        spdlog::info("scopuli::http::server waiting for all web requests to complete...");
...
auto server::make_web_request_task(coro::net::tcp_client tcp_client) -> coro::task<void>
{
    static std::atomic<uint64_t> execute_counter{0};
    uint64_t                     my_id = execute_counter++;
    ...
    while (keep_alive)
    {
        ...
        switch (pstatus)
        {
            case coro::poll_status::event:
            {
                ...
                http_request_view = std::span<char>{input_buffer.data(), http_request_view.size() + rspan.size()};
                ...
                if (rstatus == coro::net::recv_status::ok)
                {
                    ...
                    switch (parse_result)
                    {
                        case turbo::http::request_parse_result::complete:
                        {
                            ...
                            auto response_view = std::span<const char>{response_data.data(), response_data.size()};
                            ...
                            do
                            {
                                ...
                                {
                                    spdlog::error(
                                        "web_requst error sending data to client [{}]",
                                ...
                                else
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        auto pstatus = co_await tcp_client.poll(coro::poll_op::write, m_timeout);
                                        if (pstatus != coro::poll_status::event)
                                        {
                                            spdlog::error("web_request error polling for write [{}]", pstatus);
                                            keep_alive = false;
                                        }
                                    }
                                }
                            ...
                            if (keep_alive)
                            {
                                ...
                                if (connection_header.has_value() && chain::str::equal<chain::str::case_t::insensitive>(
                                                                         connection_header.value(), "keep-alive"))
                                {
                                    // Reset inputs and wait for another request.
                                    request_parser.reset();
                                    recv_view         = std::span<char>{input_buffer.data(), input_buffer.size()};
                        ...
                        case turbo::http::request_parse_result::incomplete:
                        {
                            ...
                            if (http_request_view.size() == input_buffer.size())
                            {
                                ...
                                // into.
                                recv_view = std::span<char>{
                                    input_buffer.data() + input_buffer.size() / 2, input_buffer.size() / 2};
                                http_request_view = std::span<char>{input_buffer.data(), http_request_view.size()};
                            }
                ...
                {
                    spdlog::error("web request recv() data error [{}]", coro::net::to_string(rstatus));
                    keep_alive = false;
                }
            }
            break;
