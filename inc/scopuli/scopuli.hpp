...
class request {
public:
  request(const coro::net::tcp_client &tcp_client,
          const turbo::http::request<> &request_data)
      : m_tcp_client(tcp_client), m_request_data(request_data) {}
  ~request() = default;

  auto method() const -> turbo::http::method {
    return m_request_data.http_method();
  }

  auto uri() const -> std::string_view { return m_request_data.http_uri(); }

  auto version() const -> turbo::http::version {
    return m_request_data.http_version();
  }
  ...
  auto body() const -> const std::optional<std::string_view> & {
    return m_request_data.http_body();
  }
...
class response {
public:
  response(const request &req, size_t response_size_bytes = 4096)
      : m_request(req) {
...
public:
  using request_handler =
      std::function<void(const request &req, response &rep)>;

  // using routes_type = std::map<std::string, request_handle>;
  using routes_type = request_handler;

  server(coro::net::ip_address ip_addr, uint16_t port, routes_type routes,
         processing_method pm =
             processing_method::process_requests_on_thread_pool);
...
private:
  ...
  auto make_accept_task(coro::io_scheduler &scheduler,
                        coro::net::ip_address ip_addr, uint16_t port)
      -> coro::task<void>;

  auto make_web_request_task(coro::net::tcp_client tcp_client)
      -> coro::task<void>;
